...
def get_data_size(location, data_dims):
    dims = list(map(int, data_dims.split(',')))
    d = readDat(location)
    d = torch.tensor(d).reshape((dims[2],dims[1],dims[0])).transpose(2,0)
    d = d.unsqueeze(0).unsqueeze(0)
    full_shape = d.shape[2:]
    return full_shape
...
def dat_to_tensor(location, opt = None):
    ...
    full_shape = d.shape[2:]
    return d, full_shape, d_min, d_max
...
def bin_to_tensor(location, opt = None):
    ...
    # d = d.numpy().flatten('F')
    # saveDat(d, './Data/vortex-street.raw')
    # exit()
