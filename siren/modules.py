...
class Sine(nn.Module):
    def __init__(self, omega=DEFAULT_SIREN_OMEGA):
        super().__init__()
        self.omega = omega
...
class FCBlock(MetaModule):
    ...
    def __init__(self, in_features, out_features, num_hidden_layers, hidden_features,
                 outermost_linear=False, nonlinearity='relu', weight_init=None, omega=DEFAULT_SIREN_OMEGA):
        super().__init__()
        ...
        nls_and_inits = {'sine':(Sine(omega=omega), lambda m: sine_init(m, omega), first_layer_sine_init),
...
class SingleBVPNet(MetaModule):
    ...
    def __init__(self, out_features=1, type='sine', in_features=2,
                 mode='mlp', hidden_features=256, num_hidden_layers=3,omega=DEFAULT_SIREN_OMEGA, **kwargs):
        super().__init__()
        self.mode = mode
        ...
        self.net = FCBlock(in_features=in_features, out_features=out_features, num_hidden_layers=num_hidden_layers,
                           hidden_features=hidden_features, outermost_linear=True, nonlinearity=type,omega=omega)
        print(self)
...
def sine_init(m, omega=DEFAULT_SIREN_OMEGA):
    with torch.no_grad():
        if hasattr(m, 'weight'):
            num_input = m.weight.size(-1)
