...
class Implicit2DFuncWrapper(torch.utils.data.Dataset):
    """
    Wraps a dataset that provides a scalar field "func" defined on a fixed 2D grid.
    This wrapper:
      - Ensures coords are a fixed [-1, 1]^2 grid (SIREN-friendly)
    ...
    Expected inner dataset contract:
      ...
      - __getitem__(idx) returns ({}, {'func': <Tensor[N,1]>}) where N = sidelength[0]*sidelength[1]
        and 'func' aligns with the grid produced here (i.e., same ordering).
    """
    def __init__(self, dataset, sidelength=None, compute_diff=None):
        self.dataset = dataset
        if isinstance(sidelength, int):
            sidelength = (sidelength, sidelength)
        assert sidelength is not None and len(sidelength) == 2, "sidelength must be (H, W)"

        self.sidelength = sidelength
        self.compute_diff = compute_diff
        # Always use canonical [-1,1] grid for SIREN
