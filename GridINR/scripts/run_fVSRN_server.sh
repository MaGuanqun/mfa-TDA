...
function_name=vortex_street_3d #quartic_potential_2, vortex_street
...
if [ "$function_name" = "quartic_potential_2" ]; then
    ...
    data_dim="100,80,50"
    feature_grid_shape="64,64,64"
elif [ "$function_name" = "vortex_street_3d" ]; then
    raw_data="./Data/$function_name.bin"
    data_dim="640,80,150"
    feature_grid_shape="64,64,64"
fi
...
func_raw_data="./Output/$function_name"
vti_file="./Output/$function_name"
vti_critical_point="./Output/${function_name}_critical_points.csv"
...
# do
#     python3 infer.py --model_type fVSRN --load_from ./SavedModels/$function_name --step_size $step_size
# done
...
do
    python ../src/critical_point_tracking/binary_time_data_convert.py -i "${func_raw_data}_${step_size}.dat" -o "${vti_file}-${step_size}.vti" -f "${function_name}" --step_size $step_size
