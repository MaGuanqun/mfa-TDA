...
struct Integrator
{
    ...
    //     until max_steps. We detect this EXACTLY: each new point is compared (in
    //     space-time, spatial_eps over the spatial axes and temporal_eps over
    //     time) against every earlier point on this branch, skipping the last
    //     self_skip points so the locally-adjacent path it is currently on is not
