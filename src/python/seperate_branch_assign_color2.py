...
ColorId — ColorId can repeat when there are many branches). If the closest mean
distance is <= 1e-3, copy that branch's ColorId; otherwise assign a new ColorId
randomly in [1, 40]. Existing ColorId on the input VTP is removed before
assignment.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            ...
            "to the closest branch in a reference VTP. Branches within the distance threshold "
            "reuse the reference ColorId; farther branches get a random ColorId in [1, 40]."
        )
    )
    ...
    p.add_argument(
        ...
        default=1e-3,
        help=(
            "Assign the matched reference ColorId when branch distance is <= this value; "
            "otherwise assign a random new ColorId in [1, 40] (default: 1e-3)."
        ),
    )
    p.add_argument(
        "--verbose-matches",
...
def remove_color_arrays(poly: vtk.vtkPolyData, color_array_name: str) -> None:
...
def assign_color_ids_from_reference(
    ...
    rng: random.Random,
) -> Tuple[Dict[int, Tuple[int, float, bool]], int]:
    ...
    match_info: Dict[int, Tuple[int, float, bool]] = {}

    for region_id in sorted(input_regions.keys()):
        query_pids = input_regions[region_id]
        _, matched_color_id, dist = find_closest_reference_branch(
        ...
        if dist <= match_distance_threshold:
            ...
            color_id = rng.randint(_NEW_COLOR_ID_MIN, _NEW_COLOR_ID_MAX)
            matched = False
        region_to_color_out[region_id] = color_id
        match_info[region_id] = (color_id, dist, matched)
    ...
    remove_color_arrays(poly, color_array_name)
...
def main() -> None:
    ...
    remove_color_arrays(src, args.color_array_name)
    ...
    match_info, ref_branch_count = assign_color_ids_from_reference(
        ...
        match_distance_threshold=float(args.match_distance_threshold),
        rng=rng,
    )
    ...
    matched_count = sum(1 for _, _, matched in match_info.values() if matched)
    new_count = len(match_info) - matched_count
    print(
        ...
        f"match_distance_threshold: {args.match_distance_threshold:g}"
    )
    print(f"Matched reference ColorId: {matched_count}  New ColorId (1..40): {new_count}")
    print_matches = args.verbose_matches or len(match_info) <= 50
    if print_matches:
        print(f"Matches (input RegionId -> ColorId, {dist_label}):")
        for region_id in sorted(match_info.keys()):
            color_id, dist, matched = match_info[region_id]
            status = "matched" if matched else "new"
            print(
                f"  RegionId={region_id} -> ColorId={color_id} ({status}), "
                f"{dist_label}={dist:.8g}"
            )
    else:
