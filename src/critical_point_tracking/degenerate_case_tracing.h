...
namespace degenerate_case_tracing
{
    ...
    template<typename T>
    void find_neighbor_critical_points(T time_step, T spatial_step, const Block<T>* b, const VectorX<T>& degenerate_point, std::vector<VectorX<T>>& start_points,T root_finding_grad_epsilon,std::vector<T>& same_root_epsilon,T hessian_det_epsilon,int maxIter)
    {
        auto domain_range = b->core_maxs-b->core_mins;
        std::vector<std::vector<T>> span_range_for_one_plane(domain_range.size()-1);
        std::vector<T> step_size(domain_range.size(),spatial_step);
        step_size.back() = time_step; // the last dimension is time
        ...
        for(int i=0;i<domain_range.size();++i)
        {
            for(int j=0;j<domain_range.size();++j)
            {
                ...
                //plane at degenerate_point[i]+step_size
            set_one_boudary_range(i, degenerate_point, span_range_for_one_plane, step_size, b->core_maxs, b->core_mins);
            ...
            if(b->core_maxs[i]-degenerate_point[i]>step_size[i])
            {
                fixed_value.emplace_back(degenerate_point[i]+step_size[i]);
            }
            else
            {
                fixed_value.emplace_back(b->core_maxs[i]);
            }
            if(degenerate_point[i]-b->core_mins[i]>step_size[i])
            {
                fixed_value.emplace_back(degenerate_point[i]-step_size[i]);
            }
            else
        ...
        {
            find_boundary_roots::root_finding_on_one_boundary(b, start_points, span_range[i/2], fixed_value[i], fixed_dim[i/2], root_finding_grad_epsilon, same_root_epsilon, hessian_det_epsilon, used_domain[i/2], maxIter, d_max_square[i/2], center[i/2], step_size.back(), step_size[0]);
        }

    }

    template<typename T>
    bool check_boundary_point_pass_degenerate(const VectorX<T>& degenerate_point, VectorX<T>& point, const Block<T>* b,T time_step, T spatial_step, T hessian_det_epsilon, T gradient_epsilon, int max_itr)
    {
        ...
        step[step.size()-1] = time_step; // the last dimension is time
        ...
        particle_tracing::tracing_one_direction(time_step, spatial_step, b, point, trajectory, point[point.size()-1]<degenerate_point[degenerate_point.size()-1], hessian_det_epsilon, gradient_epsilon, max_itr, d_max_square, block_min, block_max);

        //determine if the trajectory passes the degenerate point
        for(auto& p: trajectory)
        {
            if(std::abs(p[p.size()-1]-degenerate_point[p.size()-1])<time_step)
            {
                if((p.head(p.size()-1)-degenerate_point.head(degenerate_point.size()-1)).squaredNorm()<spatial_step*spatial_step)
                {
                    return true;
                }
            }
    ...
    template<typename T>
    void find_neighbor_start_points(const Block<T>* b, const VectorX<T>& degenerate_point, T ori_time_step, T ori_spatial_step, T step_ratio, T root_finding_grad_epsilon, T hessian_det_epsilon,int maxIter, int correction_itr,std::vector<VectorX<T>>& start_points)
    {
        start_points.clear();
        T time_step = ori_time_step * step_ratio;
        T spatial_step = ori_spatial_step * step_ratio;
        std::vector<T> same_root_epsilon(degenerate_point.size(), spatial_step); // same root epsilon for each dimension except time
        same_root_epsilon.back()= time_step; // the last dimension is time
        
        std::vector<VectorX<T>> critical_points;
        find_neighbor_critical_points(time_step, spatial_step, b, degenerate_point, critical_points, root_finding_grad_epsilon,same_root_epsilon,hessian_det_epsilon, maxIter);

        
        for(auto& cp: critical_points)
        {
            if(check_boundary_point_pass_degenerate(degenerate_point, cp, b, time_step, spatial_step, hessian_det_epsilon, root_finding_grad_epsilon, correction_itr))
            {
    ...
    template<typename T>
    void tracing_from_all_degenerate_points(const Block<T>* b, std::vector<VectorX<T>>& degenerate_points, std::vector<CP_Trace<T>>& trace, T time_step, T spatial_step, T step_ratio, T hessian_det_epsilon, T gradient_epsilon, int maxIter, int correction_max_itr, T d_max_square)
    {
        ...
        for(int i=0;i<degenerate_points.size();++i)
        {
            ...
            find_neighbor_start_points(b, degenerate_points[i], time_step, spatial_step, step_ratio, gradient_epsilon, hessian_det_epsilon, maxIter, correction_max_itr,temp_start_points);
