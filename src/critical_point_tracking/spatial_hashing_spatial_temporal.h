...
namespace spatial_hashing_spatial_temporal {
...
struct VectorEqual {
    bool operator()(const Eigen::VectorX<T>& a, const Eigen::VectorX<T>& b) const {
        ...
        T squaredDistance = (a.segment(size+2,size-1)-b.segment(size+2,size-1)).squaredNorm();
...
template<typename T>
void registerCell(std::vector<Eigen::VectorX<T>>& unique_root, Eigen::VectorX<T>& Position,T spatial_epsilon, T temporal_epsilon, std::unordered_set<Eigen::VectorX<T>, VectorHash<T>, VectorEqual<T>>& points_step_1,
std::vector<std::vector<size_t>>& temp_index) 
{
    size_t k1,k2;
    int j=0;
    for(int i=((Position.size()>>1)+1);i<Position.size()-1;++i)
    {
        temp_index[j].clear();
        k1=std::floor(Position[i] / (20*spatial_epsilon));
        k2=std::ceil(Position[i] / (20*spatial_epsilon));
    ...
    temp_index[j].clear();
    k1=std::floor(Position[Position.size()-1] / (20*temporal_epsilon));
    k2=std::ceil(Position[Position.size()-1] / (20*temporal_epsilon));
    temp_index[j].emplace_back(k1);
    if(k1!=k2)
        temp_index[j].emplace_back(k2);
...
std::vector<Eigen::VectorX<T>>& unique_root, T spatial_epsilon = SAME_ROOT_EPSILON, T temporal_epsilon = SAME_ROOT_EPSILON)
{
    ...
    Eigen::VectorX<T> temp(2*root[0].size()+2); //[spatial threshold, temporal threshold, current hash index, original position]
    temp[0]=spatial_epsilon;
    temp[1]=temporal_epsilon;
    ...
    {        
        ...
        registerCell(unique_root,temp,spatial_epsilon,temporal_epsilon,points_step_1,temp_index);
    }
