...
private:
    ...
    void partial_Hessian(VectorX<T>& p, MatrixX<T>& p_Hessian, int partial_deriv_index)
    {
        int n_var_dim = p.size() - 1;
    ...
    void diff_Hessian(VectorX<T>& p, std::vector<MatrixX<T>>& d_Hessian)
    {
        d_Hessian.resize(p.size());
        for(int i=0;i<p.size();i++)
        {
            partial_Hessian(p, d_Hessian[i], i);       
        }
    }
    
    void diff_Hessian_determinant(VectorX<T>& p,const MatrixX<T>& hessian, VectorX<T>& diff)
    {
        MatrixX<T> adj;
        adjugate_of_symmetric(hessian, adj);
        std::vector<MatrixX<T>> diff_Hes;
        diff_Hessian(p, diff_Hes);
