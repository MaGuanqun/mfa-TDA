...
int main(int argc, char** argv)
{
    ...
    auto start_time = std::chrono::high_resolution_clock::now();
        ...
        Degenerate_case_multi_threshold<double> tracking_degenerate_case(core_mins, core_maxs, J_threshold_list, grad_epsilon, step_size, max_itr, function_type, nullptr, &inr_model);
        ...
        std::vector<int> my_spans_index;
        my_spans.reserve(hi - lo);
        my_spans_index.reserve(hi - lo);
        for (size_t i = lo; i < hi; ++i) {
            VectorXi block_index;
            utility::obtainDomainIndex(i, block_index, number_in_every_dim);
            my_spans.push_back(block_index);
            my_spans_index.push_back(static_cast<int>(i));
        }
        // Empty file name so workers do not write temp files.
        tracking_degenerate_case.degenerate_finding(root, point_num_in_block, span_num, my_spans, my_spans_index,degenerate_point_file);

        if (world_size > 1) {
            std::vector<std::vector<VectorX<double>>> gathered(root.size());
            int dim = static_cast<int>(core_mins.size());
            for(int i=0;i<root.size();i++)
            {
                mpi_gather_roots(root[i], gathered[i], dim, world_comm);
                // std::cout<<"gathered root size "<<gathered[i].size()<<std::endl;
            }
            if (world_rank == 0)
        ...
        // if (world_rank == 0) {
        //     std::cout << root.size() << " roots before deduplication " << root[0].size() << std::endl;
        // }
        ...
        if (world_rank == 0) {
            ...
            for (const auto& r : root) {
                if (!r.empty()) { any_nonempty = true; break; }
            }
            if (!any_nonempty) exit_early = 1;
        }
        if (world_size > 1)
        ...
        // std::cout<<"root[0] size "<<root[0].size()<<std::endl;
