...
template<typename T>
class Degenerate_case_multi_threshold
{
private:
    const Block<T>* b;
    ...
    std::vector<T> degenerate_finding_epsilon_list;
    T gradient_epsilon;
    int max_itr;
    ...
    bool newton(std::vector<VectorX<T>>& result, VectorX<T>& p)
    {
        result.clear();
        int itr_num=0;
        MatrixX<T> dev_J;
        ...
        for(int i=0;i<degenerate_finding_epsilon_list.size();i++)
        {
            if(J[0]<degenerate_finding_epsilon_list[i]* hessian_norm && J.tail(J.size()-1).squaredNorm()<gradient_epsilon*gradient_epsilon)
            {
        ...
        if (result.size() == degenerate_finding_epsilon_list.size())
        {
            return true;
        }


        int start_epsilon_index = result.size();
        ...
        while(itr_num<max_itr)
        {
            ...
            if(itr_num>0){
                for(int i=start_epsilon_index;i<degenerate_finding_epsilon_list.size();i++)
                {
                    if(J[0]< degenerate_finding_epsilon_list[i]* hessian_norm 
                    && J.tail(J.size()-1).squaredNorm()<gradient_epsilon*gradient_epsilon
                    ){                    
                        result.emplace_back(p);
                    }
                    else{
                        start_epsilon_index = i;
                        break;
                    }
                }
                if (result.size() == degenerate_finding_epsilon_list.size())
   ...
   // Function to find the roots of the polynomial using Newton's method
    ...
    bool degenerate_finding_single_block(std::vector<std::vector<VectorX<T>>>& root_list,
        std::vector<std::vector<T>>&initial_point, std::vector<std::array<int,2>>& initial_point_range)
    {
        ...
        std::vector<VectorX<T>> next_root_list; 
        ...
        for(int i=0;i<num_initial_point;++i)
        {
            ...
            if(newton(next_root_list, current_initial_point))
            {
        
                if(tracking_utility::newRootList(next_root_list,root_list,same_root_epsilon))
                {        
                    root_list.emplace_back(next_root_list);
                } 
            }
...
Degenerate_case_multi_threshold(const VectorX<T>& core_mins_, const VectorX<T>& core_maxs_, std::vector<T>& degenerate_finding_epsilon_list_, T gradient_epsilon_,std::vector<T> same_root_epsilon_, int max_itr_=50,  const int function_type_=0, const Block<T>* b_=nullptr, INRModel<T>* inr_model_=nullptr)
    : domain_min(core_mins_), domain_max(core_maxs_), b(b_), function_type(function_type_), degenerate_finding_epsilon_list(degenerate_finding_epsilon_list_), gradient_epsilon(gradient_epsilon_), max_itr(max_itr_), same_root_epsilon(same_root_epsilon_), inr_model(inr_model_)
    {
    ...
    ~Degenerate_case_multi_threshold(){}


void rearrange_points_from_different_epsilon(std::vector<std::vector<VectorX<T>>>& ori_root_list, std::vector<std::vector<VectorX<T>>>& re_arranged_root_list)
//from [[epsilon1, epsilon2, epsilon3, ...],[],[]] to [[epsilon1], [epsilon2], [epsilon3], ...]
{
    re_arranged_root_list.clear();
    re_arranged_root_list.resize(degenerate_finding_epsilon_list.size());
    for(int i=0;i<ori_root_list.size();i++)
    {
        for(int j=0;j<ori_root_list[i].size();j++)
        {
            if(ori_root_list[i][j].size()!=0)
            {
                re_arranged_root_list[j].emplace_back(ori_root_list[i][j]);
            }
        }
    }
}

void write_degenerate_point(std::vector<std::vector<VectorX<T>>>& to_record_root, string degenerate_point_file, int block_index, int num_block)
{
    ...
    std::vector<std::vector<VectorX<T>>> re_arranged_root_list;

    std::vector<MatrixX<T>> root_matrix(degenerate_finding_epsilon_list.size());

    rearrange_points_from_different_epsilon(to_record_root, re_arranged_root_list);


    for(int i=0;i<re_arranged_root_list.size();i++)
    {
        if(re_arranged_root_list[i].empty())
        {
            root_matrix[i].resize(0,0);
            continue;
        }
        root_matrix[i].resize(re_arranged_root_list[i].size(),re_arranged_root_list[i][0].size());
        for(int j=0;j<re_arranged_root_list[i].size();j++)
        {
            root_matrix[i].row(j) = re_arranged_root_list[i][j].transpose();
        }
    }

    string degenerate_file_name = degenerate_point_file + "-temp_record-" + std::to_string(int(block_index)) + ".dat";

    to_record_root.clear();
    utility::writeMatrixVector(degenerate_file_name.c_str(),root_matrix);

    // std::cout<<"degenerate case processing block "<<block_index <<" "<<(float)block_index/(float)num_block<<std::endl;
}

    
    void degenerate_finding(std::vector<std::vector<VectorX<T>>>& root,
    const VectorXi& point_num_in_block, const VectorXi& set_block_num, 
    const std::vector<VectorXi>& selected_span_index = std::vector<VectorXi>(),
    const std::vector<int>& my_spans_index = std::vector<int>(),
    string degenerate_point_file = "")
    {
        root.clear();
        root.resize(degenerate_finding_epsilon_list.size());
        ...
        int num_block;
        int total_num_block;
        total_num_block =set_block_num.prod();
        ...
        tbb::enumerable_thread_specific<std::vector<std::vector<VectorX<T>>>> local_root;
        ...
        [&](const tbb::blocked_range<size_t>& range)
        {
            ...
            std::vector<std::vector<VectorX<T>>> block_root_list;
            std::vector<std::vector<VectorX<T>>> to_record_root;
        ...
        for (const auto& thread_vec : local_root) {
            combined_root_list.insert(combined_root_list.end(), thread_vec.begin(), thread_vec.end());
        }

        std::vector<std::vector<VectorX<T>>> re_arranged_root_list;
        rearrange_points_from_different_epsilon(combined_root_list, re_arranged_root_list);
