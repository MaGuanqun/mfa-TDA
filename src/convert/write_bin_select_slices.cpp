...
std::vector<int>& upsample_factor, std::vector<T>& shrink_range_raio, int start_slice_index, int end_slice_index)
{
    ...
    [&](const tbb::blocked_range<size_t>& interval)
    {
        ...
        for(size_t j=interval.begin();j<interval.end();++j)
        {
            ...
            domain_index_(dom_dim-1)+=start_slice_index;
        //Only support 2D
            VectorX<T> result_value(block->mfa->nvars());
...
void write_vtk_files(
        ...
        int output_gradient_magnitude, bool set_zero, int start_slice_index, int end_slice_index,  bool use_bin=false)                     // science variable to render geometrically for 1d and 2d domains
{
...
else
{
    if(use_bin)
    {
        save_bin(function_filename,b,dom_dim,pt_dim,upsample_factor,shrink_range_raio, start_slice_index, end_slice_index);
...
int main(int argc, char ** argv)
{
    ...
    ops >> opts::Option('x', "start slice index",    start_slice_index,       " start slice index for 3D data");
    ops >> opts::Option('y', "end slice index",    end_slice_index,       " end slice index for 3D data");
    ...
    master.foreach([&](Block<real_t>* b, const diy::Master::ProxyWithLink& cp)
            { 
                write_vtk_files(b, cp, sci_var, upsample_factor,shrink_ratio,ignore, output_obj_name,output_vtk_name,output_gradient_magnitude, set_zero,start_slice_index,end_slice_index, use_bin); });
}
