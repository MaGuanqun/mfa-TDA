...
// ---------------------------------------------------------------------------
template<typename T>
void save_vff(string& file_name, Block<real_t>* block, size_t dom_dim,
std::vector<int>& upsample_factor)
{
    auto& tc = block->mfa->var(0).tmesh.tensor_prods[0];
    VectorXi span_num = tc.nctrl_pts - block->mfa->var(0).p;
    ...
    [&](const tbb::blocked_range<size_t>& interval)
    {
        ...
        for (size_t j = interval.begin(); j < interval.end(); ++j)
        {
            ...
            for (int m = 0; m < (int)dom_dim; m++)
            {
                ...
                if (c < block->core_mins(m)) c = block->core_mins(m);
                if (c > block->core_maxs(m)) c = block->core_maxs(m);
                coordinate[m] = c;
            }
    ...
    if (!out)
    {
        std::cerr << "save_vff: cannot open " << file_name << " for writing" << std::endl;
        return;
    }
    out.write("VFF1", 4);
    ...
    std::vector<double> mn(dom_dim), mx(dom_dim);
    for (int a = 0; a < (int)dom_dim; a++) { mn[a] = (double)block->core_mins(a); mx[a] = (double)block->core_maxs(a); }
...
void write_vtk_files(
        ...
        int output_gradient_magnitude, bool set_zero, bool use_bin=false,
        string output_vff_name="")                     // science variable to render geometrically for 1d and 2d domains
{
    // Feature Flow Fields: write the spatial-gradient vector field and return.
    if (!output_vff_name.empty())
    {
        save_vff<real_t>(output_vff_name, b, (size_t)b->mfa->dom_dim, upsample_factor);
        return;
    }
...
int main(int argc, char ** argv)
{
    ...
    string output_vff_name;     // if set, write .vff spatial-gradient vector field for FFF
    ...
    ops >> opts::Option('w', "output vff",    output_vff_name,       " output .vff spatial-gradient vector field for feature flow fields");
    ...
    master.foreach([&](Block<real_t>* b, const diy::Master::ProxyWithLink& cp)
            { 
                write_vtk_files(b, cp, sci_var, upsample_factor,shrink_ratio,ignore, output_obj_name,output_vtk_name,output_gradient_magnitude, set_zero,use_bin, output_vff_name); });
}
