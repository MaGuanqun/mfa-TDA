...
namespace find_root_jacobi_set
{    
    ...
    {
        grad.resize(p.size());
        VectorX<T> f_vector(b->mfa->nvars());
    ...
    template<typename T>
    int gradient_descent_compute_h_gradient_h(VectorX<T>& p, const Block<T>* b,
    const Block<T>* b2,
    VectorX<T>& h_first_deriv, T& h,T value, T threshold)
    {
        VectorX<T> f_first_deriv(p.size());
        compute_gradient(b,p,f_first_deriv);

        VectorX<T> g_first_deriv(p.size());
        compute_gradient(b2,p,g_first_deriv);
    ...
    bool gradient_descent_h(const Block<T>* b,
    const Block<T>* b2,
    VectorX<T>& result, VectorX<T>& p, int max_itr,
                ...
                T root_finding_epsilon, T& function_value)
    {
        ...
        while(i<max_itr)
        {
            ...
            if(gradient_descent_compute_h_gradient_h(p,b,b2,grad,function_value,0.0,root_finding_epsilon))
            {
    ...
    template<typename T>
    bool root_finding(const Block<T>* b,
    const Block<T>* b2,
    VectorXi& span_index, std::vector<VectorX<T>>& root,
        ...
        for(int i=0;i<num_initial_point;++i)
        {
            ...
            if(gradient_descent_h(b, b2, next_root, current_initial_point,maxIter,span_range,d_max_square,center,root_finding_epsilon,func_value))
            {
    ...
    template<typename T>
    bool root_finding(Block<T>* block,Block<T>* block2,
    std::vector<VectorX<T>>& root,VectorXi& number_in_every_domain,
        int current_index,
        T root_finding_epsilon, std::vector<T>& function_value,T same_root_epsilon) //2^n+1 initial points) 
    {
        VectorXi span_index(block->dom_dim);
        

        // for(auto i=0;i<block->vars.size();++i)
        // {            
            utility::obtainDomainIndex(current_index,span_index,number_in_every_domain);
            span_index+=block->mfa->var(0).p;

            if(root_finding(block,block2,span_index, root,
