// for Jacobi set, find the critical point of h when h=0
...
namespace jacobi_set_h_critical_point
{
...
template<typename T>
void compute_h(const Block<T>* b, VectorX<T>& p0, VectorX<T>& h,
        const Block<T>* b2)
{
    ...
    for(int i=0;i<p0.size();i++)
    {
        deriv.setZero();
        deriv[i]+=1;
        mfa_extend::recover_mfa(b,p0,dev_f_vector, deriv);
    ...
    for(int i=0;i<p0.size();i++)
    {
        deriv.setZero();
        deriv[i]+=1;
        mfa_extend::recover_mfa(b2,p0,dev_f_vector,deriv);
...
template<typename T>
void compute_h_dev_h(const Block<T>* b, VectorXi& span_index, VectorX<T>& p0, VectorX<T>& h, MatrixX<T>& h_first_deriv,
                const Block<T>* b2)
{
    VectorX<T> f_first_deriv;
    ...
    for(int i=0;i<span_index.size();i++)
    {
        deriv.setZero();
        deriv[i]+=1;
        mfa_extend::recover_mfa(b, p0,f_vector,deriv);
    ...
    for(int i=0;i<span_index.size();i++)
    {
        deriv.setZero();
        deriv[i]+=1;
        mfa_extend::recover_mfa(b2, p0,f_vector,deriv);
...
template<typename T>
void newton_itr(const Block<T>* b, std::vector<T>& result,T p, int max_itr)
{
    result.clear();
    T f,dev_f;
    int i=0;
...
template<typename T>
bool newton(const Block<T>* b,VectorXi& span_index, VectorX<T>& result, VectorX<T>& p, int max_itr,
                std::vector<std::vector<T>>& span_range,
                T d_max_square, VectorX<T>& center,
                bool& filtered_out, size_t& itr_num, T root_finding_epsilon, const Block<T>* b2)
{
    itr_num=0;
    ...
    MatrixX<T> dev_h;
    VectorX<T> h;
    compute_h_dev_h(b,span_index,p,h,dev_h, b2);
...
template<typename T>
void newtonSolve(const Block<T>* b, VectorXi& span_index, std::vector<VectorX<T>>& root, int& original_root_size, 
    size_t& filtered_out_num,T same_root_threshold, size_t& itr_num, T root_finding_epsilon, int maxIter, const Block<T>* b2) { 
    ...
    VectorX<T> local_domain_range = b->core_maxs-b->core_mins;

    VectorX<T> center(span_index.size());
    for(int i=0;i<span_index.size();++i)
    {    
        span_range[i].emplace_back(b->mfa->var(0).tmesh.all_knots[i][span_index[i]]*local_domain_range[i]+b->core_mins[i]);
        span_range[i].emplace_back(b->mfa->var(0).tmesh.all_knots[i][span_index[i]+1]*local_domain_range[i]+b->core_mins[i]);
    ...
    utility::compute_initial_points_js(initial_point,b->mfa->var(0).p,span_range);
    ...
    for(int i=0;i<num_initial_point;++i)
    {
        ...
        bool find_root=newton(b, span_index, next_root, current_initial_point,maxIter,span_range,d_max_square,center,filetered_out,current_itr_num,root_finding_epsilon, b2);
    ...
    int& original_root_size, size_t& filtered_out_num, T same_root_threshold, size_t& itr_num, T root_finding_epsilon, int maxItr,Block<T>* block2) //(p+1)^d initial points) 
{
    ...
    {
        newtonSolve(block, span_index[i][current_index], root, 
        original_root_size,
       filtered_out_num,same_root_threshold,itr_num,root_finding_epsilon,maxItr,block2);
    }
}
...
template<typename T>
void filterRoot(const Block<T>* b, std::vector<VectorX<T>>& root,std::vector<VectorX<T>>& new_root, std::vector<VectorX<T>>& value0, T threshold,const Block<T>* b2)
{
...
int compute_index(const Block<T>* b,VectorX<T>& root, T threshold)
{
    ...
    VectorX<T> deriv_value(b->mfa->nvars());    
