...
using namespace std;
    ...
    void set_dom_bounds(Bounds<real_t>& dom_bounds, string input)
    {
        ...
        else if(input == "sinc_sum" || input == "sinc_sum_2")
        {
            for (int i = 0; i < dom_bounds.min.dimension(); i++)
            {
                dom_bounds.min[i] = -2.0 * M_PI;
                dom_bounds.max[i] =  2.0 * M_PI;
            }
        }
        else if(input == "rastrigin")
        {
            for (int i = 0; i < dom_bounds.min.dimension(); i++)
            {
                dom_bounds.min[i] = -6.0;
        ...
        else if(input == "ackley")
        {
            for (int i = 0; i < dom_bounds.min.dimension(); i++)
            {
                dom_bounds.min[i] = -2.0;
                dom_bounds.max[i] = 2.0;
            }
        }
        else if(input == "schwefel")
        {
            for (int i = 0; i < dom_bounds.min.dimension(); i++)
            {
                dom_bounds.min[i] = -10.5 * M_PI * 10.5*M_PI;
        ...
        else if(input=="cylinder2")
        {
            dom_bounds.min = { 3.2,  0.5, 0.0};
            dom_bounds.max = { 5.5,  1.5, 1.0};
        }
        else if(input=="cylinder3")
        {
            dom_bounds.min = { 3.5,  0.5, 0.0};
            dom_bounds.max = { 5.5,  1.5, 1.0};
        }
        else if(input=="vortex")
        {
            dom_bounds.min = {0, 0, 0,0.0};
            dom_bounds.max = {127,127,127,89};
        }
        else
        {
                        ...
                        mfa::MFAInfo& mfa_info, DomainArgs& d_args)
    {
        ...
        {
            d_args.full_dom_pts = {640, 80, 150};
            d_args.ndom_pts = d_args.full_dom_pts;
        ...
        if (input == "vortex")
        {
            d_args.full_dom_pts = {128, 128, 128, 90};
            d_args.ndom_pts = d_args.full_dom_pts;

            if (!adaptive)
            {
                vars_nctrl = {18, 18, 18, 13};
            }   

            d_args.min.resize(4);
            d_args.max.resize(4);
            d_args.min[0] = 0.0;    d_args.max[0] = 127.0;
            d_args.min[1] = 0.0;    d_args.max[1] = 127.0;
            d_args.min[2] = 0.0;    d_args.max[2] = 127.0;
            d_args.min[3] = 0.0;    d_args.max[3] = 89.0;
            d_args.set_domain_range=true;
        ...
        if (input == "boussinesq_3d")
        {
            ...
            d_args.min[2] = 0.0;    d_args.max[2] = 1.5;
            d_args.set_domain_range=true;
